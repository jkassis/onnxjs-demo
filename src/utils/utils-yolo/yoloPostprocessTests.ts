...
function softmaxTest() {
    ...
    const expected = new Tensor('float32', Float32Array.from([0.0158762, 0.1173104, 0.8668135,
        0.0900306, 0.2447284, 0.6652408]), [2, 3]);
    if (!assertTensorEquality(actual, expected)) {
...
function sliceTest() {
    const actual = tensorTransformUtils.slice(
        new Tensor('int32', Int32Array.from([1, 2, 3, 4]), [2, 2]), [1, 0], [1, 2]);
...
function greaterEqualTest() {
    const actual = tensorTransformUtils.greaterEqual(
        new Tensor('float32', [1, 2, 3]), new Tensor('float32', [2, 2, 2]));
